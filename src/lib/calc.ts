 ...
 */
function averageTimedStat(statAmount: number, cooldown?: number, duration?: number) {
 ...
 */
function calculateItemStatPriority(item: Item, priority: StatPriority): number {
    let result = 0;
...
export function calculateAllItemPriorities(
    items: Item[],
    statPriority: StatPriority,
    conditionPriority: ConditionPriority,
) {
